...
class StepLogger:
    ...
    def __init__(self, steps: list[str], console: Console | None = None) -> None:
        """Initialize step logger with list of step names."""
        self.steps = steps
    ...
    def __exit__(self, exc_type: type[BaseException] | None, exc_val: BaseException | None, exc_tb: object) -> None:
        ...
        warning_count = sum(1 for step_info in self._completed_steps.values() if step_info.get("warning", False))
        error_count = sum(1 for step_info in self._completed_steps.values() if step_info.get("error", False))

        if error_count == 0 and warning_count == 0:
            self.console.print(
                f"\n[green]✅ All {len(self.steps)} steps completed successfully in {total_time:.1f}s[/green]"
            )
        elif error_count == 0:
            self.console.print(
                f"\n[yellow]⚠️  {success_count}/{len(self.steps)} steps completed with {warning_count} warnings in {total_time:.1f}s[/yellow]"
            )
        else:
            self.console.print(
                f"\n[red]❌ {success_count}/{len(self.steps)} steps completed with {error_count} errors in {total_time:.1f}s[/red]"
            )
    ...
    def get_error_count(self) -> int:
        ...
        return sum(1 for step_info in self._completed_steps.values() if step_info.get("error", False))
